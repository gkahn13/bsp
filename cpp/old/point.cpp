...
void beliefCollocation(std::vector< Matrix<B_DIM> >& B, std::vector< Matrix<U_DIM> >& U, beliefMPC_params& problem, beliefMPC_output& output, beliefMPC_info& info)
{
	...
	for(int it = 0; it < maxIter; ++it) 
	{
		...
		for (int t = 0; t < T-1; ++t) 
		{
			...
			// Fill in lb, ub, C, e
			// G: 0-1 for x, 2-4 for sigma, 5-6 for u
			...
			if (t > 0) { // G: dynamics equality constraint
				...
				int idx = 0;
				for(int c = 0; c < (B_DIM+U_DIM); ++c) { // G: this loop to create transpose?
				...
				// G: why isn't it
				// eVec = -h[t]
				// because currently, it seems like the equations lead to
				// F_i*b_i + G_i*u_i - b_{i+1} = -h_i + F_i*b_i + G_i*u_i
				// leading to
				// b_{i+1} = h_{i}
				// while I thought it should be
				// F_i*b_i + G_i*u_i + h_i = b_{i+1}
