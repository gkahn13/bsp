...
def plot_belief_trajectory(B, bDim, xDim, T, camera_range, camera_view_angle):
    plt.clf()
    plt.cla()
    
    B = np.array(B)
    B = ml.matrix(B.reshape(bDim, T))
    
    numLandmarks = (xDim - 3) / 2
    ...
    for t in xrange(0,T):
        ...
        X[:,t], SqrtSigma_t = decompose_belief(B[:,t], bDim, xDim)
        ...
        side_length = camera_range / math.cos(camera_view_angle)

        p1 = [X_car[0,0] + side_length*math.cos(X[2, t]-camera_view_angle), X_car[1,0] + side_length*math.sin(X[2, t]-camera_view_angle)]
        p2 = [X_car[0,0] + side_length*math.cos(X[2, t]+camera_view_angle), X_car[1,0] + side_length*math.sin(X[2, t]+camera_view_angle)]

        plt.plot([X_car[0,0], p1[0], p2[0], X_car[0,0]],
                 [X_car[1,0], p1[1], p2[1], X_car[1,0]],
                 color='red')
