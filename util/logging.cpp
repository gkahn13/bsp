...
int LoggingInit() {
	...
	char* lvlc = getenv("BSP_LOG_THRESH");
	string lvlstr;
	if (lvlc == NULL) {
		printf("You can set logging level with BSP_LOG_THRESH. Valid values: %s. Defaulting to INFO\n", VALID_THRESH_VALUES);
		lvlstr = "INFO";
	}
	...
	if (lvlstr == "FATAL") gLogLevel = LevelFatal;
		...
		printf("Invalid value for environment variable BSP_LOG_THRESH: %s\n", lvlstr.c_str());
