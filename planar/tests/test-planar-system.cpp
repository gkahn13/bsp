...
void test_beams() {
	...
	vec<J_DIM> j;
//	j << -M_PI/8, M_PI/4, -M_PI/1.5, M_PI/16;
//	j << 0.625051,  0.592922, -0.221204,  0.712487;
//	sys.display(j);
//	return;

	for(int i=0; i < 100; ++i) {
		j << planar_utils::uniform(-M_PI/3, M_PI/3),
...
//	std::vector<Beam> fov = sys.get_fov(j);
//	std::vector<Segment> link_segments = sys.get_link_segments(j.segment<E_DIM>(0));
...
void test_entropy() {
	...
	for(int m=0; m < M_DIM; ++m) {
		P(0, m) = planar_utils::uniform(-10, 10);
		P(1, m) = planar_utils::uniform(2, 11);
	}
	...
	std::vector<vec<J_DIM>, aligned_allocator<vec<J_DIM>>> J(T, j0);

	double alpha = 1;
	double entropy = sys.cost_entropy(J, U, P, alpha);
