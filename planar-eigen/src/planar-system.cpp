...
PlanarSystem::PlanarSystem(const vec<2>& camera_origin, const vec<2>& object, bool is_static) {
	init(camera_origin, object, is_static);
}

void PlanarSystem::init(const vec<2>& camera_origin, const vec<2>& object, bool is_static) {
...
vec<Z_DIM> PlanarSystem::obsfunc(const vec<X_DIM>& x, const vec<2>& object, const vec<R_DIM>& r) {
	vec<Z_DIM> z;
	z(0) = x(0); // joint 0
	z(1) = x(1); // joint 1
	z(2) = x(2); // joint 2
...
mat<Z_DIM,Z_DIM> PlanarSystem::delta_matrix(const vec<X_DIM>& x, const vec<2>& object, const double alpha) {
...
double PlanarSystem::cost(const std::vector<vec<X_DIM>, aligned_allocator<vec<X_DIM>>>& X, const mat<X_DIM,X_DIM>& sigma0,
	...
	vec<2> final_ee_position = get_link_segments(x_tp1).back().p1;
	vec<2> goal_error = final_ee_position - x_tp1.segment<C_DIM>(J_DIM);
