...
SXMatrix costfunc(const SXMatrix& XU, const SXMatrix& Sigma_0, const SXMatrix& params, const SXMatrix& cam0, const SXMatrix& cam1)
{
	...
	x_goal = XU(Slice(offset,offset+X_DIM));
	offset += X_DIM;
	...
	cost += params[3]*inner_prod(pos_goal - pos_comp, pos_goal - pos_comp);
...
int main()
{
	...
	// Generate code
	generateCode(f_fcn,"arm-control-cost");
	generateCode(grad_f_fcn,"arm-control-grad");
	...
	t_x2[0] = 10; t_x2[1] = 1; t_x2[2] = 10; t_x2[3] = 100; 
	for(int i = 0; i < 3; ++i) {
		t_x3[i] = c0[i];
		t_x4[i] = c1[i];
	}
