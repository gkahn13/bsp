function arm_belief_pos_penalty_mpc_gen(timesteps)
...
% cost
params(1) = newParam(['H',istr], i, 'cost.H');
...
for i = 2:N
    ...
    if i < N
        ...
        stages(i).dims.r = nb+ng;      % number of equality constraints
    end
    params(end+1) = newParam(['C',istr], i, 'eq.C');
    params(end+1) = newParam(['e',istr], i, 'eq.c');
    stages(i).eq.D = [-eye(nb), zeros(nb,2*nb+nu)];
...
% dimension
stages(i).dims.n = nb+2*ng;    % number of stage variables
stages(i).dims.r = nb+ng;    % number of equality constraints
stages(i).dims.l = nb+2*ng;    % number of lower bounds
