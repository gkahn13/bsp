...
void test_truncate_gaussian() {
	...
	int num_examples = 5; // depends on number pushed back below
	std::vector<vec<X_DIM>, aligned_allocator<vec<X_DIM>>> X_initial(num_examples);
	std::vector<mat<X_DIM,X_DIM>, aligned_allocator<mat<X_DIM,X_DIM>>> S_initial(num_examples);

	X_initial[0] << M_PI/3, -M_PI/2, -M_PI/16, M_PI/8, 2, 5;
	S_initial[0] = 20*mat<X_DIM,X_DIM>::Identity();

	X_initial[1] << M_PI/2.5, -M_PI/2, -M_PI/3, 0, 2, 5;
	S_initial[1] = 20*mat<X_DIM,X_DIM>::Identity();

	X_initial[2] << M_PI/3, -M_PI/2, -M_PI/16, -M_PI/8, 2, 5;
	S_initial[2] = 20*mat<X_DIM,X_DIM>::Identity();

	X_initial[3] << M_PI/2, 0, 0, 0, .05, 5;
	S_initial[3] = 20*mat<X_DIM,X_DIM>::Identity();

	X_initial[4] << M_PI/3, -M_PI/2, -M_PI/16, -M_PI/6, 2, 5;
	S_initial[4] = 20*mat<X_DIM,X_DIM>::Identity();
