...
double computeCost(const std::vector< Matrix<X_DIM> >& X, const std::vector< Matrix<U_DIM> >& U, int t_start)
{
	double cost = 0;

	Matrix<B_DIM> b = zeros<B_DIM>();
	Matrix<X_DIM> x = X[0];
	Matrix<X_DIM, X_DIM> SqrtSigma = identity<X_DIM>();
...
inline double costStub(const std::vector< Matrix<X_DIM> >& X, const std::vector< Matrix<U_DIM> >& U, int t_start)
{
	//int nvars = (int)maskIndices.size();
	//vars = new double[nvars];
	...
	//return resultCost[0];
	return computeCost(X,U,t_start);
}

void finiteDifferenceGradientHessian(std::vector< Matrix<X_DIM> >& X, std::vector< Matrix<U_DIM> >& U, std::vector< Matrix<X_DIM+U_DIM> >& gradients, std::vector< Matrix<X_DIM+U_DIM> >& diag_hessians) {

	double cost, cost_plus, cost_minus;

	//cost = costStub(X,U,0);

	//std::cout << std::setprecision(10) << cost << std::endl;

	//std::cout << std::setprecision(10) << computeCost(X,U) << std::endl;
	...
	for(int t = 0; t < TIMESTEPS; ++t) {
		...
		for(int i = 0; i < X_DIM+U_DIM; ++i) {
			...
			if (i < X_DIM) {
				...
				cost_plus = costStub(X,U,t);

				X[t][i] = original;

				X[t][i] -= step;
				cost_minus = costStub(X,U,t);
			...
			gradients[t][i] = (cost_plus - cost_minus)/(2*step);
...
double stateCollocation(std::vector< Matrix<X_DIM> >& X, std::vector< Matrix<U_DIM> >& U, stateMPC_params& problem, stateMPC_output& output, stateMPC_info& info)
{
	...
	std::vector< Matrix<X_DIM+U_DIM> > gradients(T);
	std::vector< Matrix<X_DIM+U_DIM> > diag_hessians(T);
	...
	for(int it = 0; it < maxIter; ++it)
		{
			...
			if (solution_accepted) {
				...
				finiteDifferenceGradientHessian(X, U, gradients, diag_hessians);
				merit = costStub(X,U,0);
				...
				for (int t = 0; t < T-1; ++t) {
					for(int i = 0; i < (X_DIM+U_DIM); ++i) {
						H[t][i] = diag_hessians[t][i];
					}
				}
				H[T-1][0] = diag_hessians[T-1][0];
				H[T-1][1] = diag_hessians[T-1][1];
				...
				f[T-1][0] = gradients[T-1][0];
				f[T-1][1] = gradients[T-1][1];
			...
			new_merit = costStub(Xopt, Uopt, 0);
