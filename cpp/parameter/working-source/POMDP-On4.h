...
inline void computeCEFJ(const Dynamics& experiment, double step,
	...
	Matrix<1,_xDim>& tTC, Matrix<1,_uDim>& tTE, Matrix<1,_xDim>& vecSF, Matrix<1,_uDim>& vecSJ)
{
	...
	for (size_t i = 0; i < _xDim; ++i) {
		...
		experiment.linearizeDynamics(step, xR, uBar, xNext, A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_M);
		experiment.linearizeObservation(step, xNext, uBar, H, N);
		beliefDynamics(A, M, H, N, SigmaBar, GammaR, WR);

		experiment.linearizeDynamics(step, xL, uBar, xNext, A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_M);
		experiment.linearizeObservation(step, xNext, uBar, H, N);
	...
	for (size_t i = 0; i < _uDim; ++i) {
		...
		experiment.linearizeDynamics(step, xBar, uR, xNext, A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_M);
		experiment.linearizeObservation(step, xNext, uR, H, N);
		beliefDynamics(A, M, H, N, SigmaBar, GammaR, WR);

		experiment.linearizeDynamics(step, xBar, uL, xNext, A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_M);
		experiment.linearizeObservation(step, xNext, uL, H, N);
		beliefDynamics(A, M, H, N, SigmaBar, GammaL, WL);
...
template <size_t _xDim, size_t _uDim, size_t _zDim>
inline void controlPolicy(const Dynamics& experiment, double step,
	const Matrix<_xDim>& xBar, const SymmetricMatrix<_xDim>& SigmaBar, const Matrix<_uDim>& uBar,
	SymmetricMatrix<_xDim>& S, Matrix<1,_xDim>& sT, Matrix<1,_sDim>& tT, Matrix<_uDim,_xDim>& L, Matrix<_uDim>& l, double& gradient) 
{
	...
	experiment.linearizeDynamics(step, xBar, uBar, xNext, A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_B|COMPUTE_M); // O(n^3)

	Matrix<_zDim, _xDim> H;
	SymmetricMatrix<_zDim> N;

	experiment.linearizeObservation(step, xNext, uBar, H, N); // O(n^3);
	...
	computeCEFJ<_xDim, _uDim, _zDim>(experiment, step, xBar, SigmaBar, uBar, tT, hvecS, tTC, tTE, hvecSF, hvecSJ); // O(n^4)
	...
	experiment.quadratizeCost(step, xBar, SigmaBar, uBar, q, Q, R, P, qT, rT, pT, COMPUTE_Q|COMPUTE_R|COMPUTE_P|COMPUTE_qT|COMPUTE_rT|COMPUTE_pT);
...
template <size_t _xDim, size_t _uDim, size_t _zDim>
inline void backwardIteration(const Dynamics& experiment, double step,
	...
	std::vector<Matrix<_uDim, _xDim> >& L, std::vector<Matrix<_uDim> >& l, double& gradient)
{
	...
	experiment.quadratizeFinalCost(xBar.back(), SigmaBar.back(), s, S, sT, tT, COMPUTE_S|COMPUTE_sT|COMPUTE_tT);
	gradient = 0.0;

	for (size_t t = uBar.size() - 1; t != -1; --t) {
		//controlPolicy(experiment, step, linearizeDynamics, linearizeObservation, quadratizeCost, xBar[t], SigmaBar[t], uBar[t], S, sT, tT, L[t], l[t], gradient);
		controlPolicy<_xDim,_uDim,_zDim>(experiment, step, xBar[t], SigmaBar[t], uBar[t], S, sT, tT, L[t], l[t], gradient);
	}
}

template <size_t _xDim, size_t _uDim>
inline void expectedCost(const Dynamics& experiment, double step,
	...
	SymmetricMatrix<_xDim>& S, double& s) 
{
	...
	experiment.linearizeDynamics(step, xBar, uBar, c, A, B, M, COMPUTE_A|COMPUTE_B);
	bool retcode = experiment.quadratizeCost(step, xBar, SigmaBar, uBar, q, Q, R, P, qT, rT, pT, COMPUTE_q|COMPUTE_Q|COMPUTE_R|COMPUTE_P);
...
template <size_t _xDim, size_t _uDim>
inline double computeExpectedCost(const Dynamics& experiment, double step,
	...
	const std::vector<Matrix<_xDim> >& xBar, const std::vector<SymmetricMatrix<_xDim> >& SigmaBar, const std::vector<Matrix<_uDim> >& uBar, const std::vector<SymmetricMatrix<_xDim> >& WBar) 
{
	...
	//CAL_DestroyGroup(cal_test);
	//CAL_CreateGroup(&cal_test, 0, false);
	//int meanid;
	//for (size_t t = pathLen - 1; t != -1; --t) {
	//	CAL_CreateSphere(cal_test, 0.1f, (float)xBar[t][0], (float)xBar[t][1], 0.0f, &meanid);
	//	CAL_SetObjectColor(meanid, 0.0f, 0.0f, 1.0f);
	//}
	...
	experiment.quadratizeFinalCost(xBar[pathLen], SigmaBar[pathLen], s, S, sT, tT, COMPUTE_s|COMPUTE_S);
	for (size_t t = pathLen - 1; t != -1; --t) {
		//expectedCost(experiment, step, linearizeDynamics, quadratizeCost, xBar[t], SigmaBar[t], uBar[t], WBar[t], L[t], S, s);
		expectedCost(experiment, step, xBar[t], SigmaBar[t], uBar[t], WBar[t], L[t], S, s);
	}
...
template <size_t _xDim, size_t _uDim, size_t _zDim>
inline void integrateControlPolicy(const Dynamics& experiment, double step,
	...
	std::vector<Matrix<_xDim> >& xNext, std::vector<SymmetricMatrix<_xDim> >& SigmaNext, std::vector<Matrix<_uDim> >& uNext, std::vector<SymmetricMatrix<_xDim> >& WNext)
{
	...
	for (size_t t = 0; t < pathLen; ++t) {
		...
		experiment.saturateControl(uNext[t]);
		
		experiment.linearizeDynamics(step, xNext[t], uNext[t], xNext[t+1], A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_M);
		experiment.linearizeObservation(step, xNext[t+1], uBar[t], H, N);
		beliefDynamics(A, M, H, N, SigmaNext[t], SigmaNext[t+1], WNext[t]);
	}
}

template <size_t _xDim, size_t _uDim, size_t _zDim>
inline void forwardIteration(const Dynamics&experiment, double step,
	...
	double& bestCost, double& eps, double gradient)
{
	...
	cost = computeExpectedCost(experiment, linearizeDynamics, quadratizeFinalCost, quadratizeCost,
	...
	//bestCost = computeExpectedCost(experiment, step, linearizeDynamics, quadratizeFinalCost, quadratizeCost, L,
		//xBar, SigmaBar, uBar, WBar);
	bestCost = computeExpectedCost<_xDim,_uDim>(experiment, step, L, xBar, SigmaBar, uBar, WBar);
	bestEps = 0.0;

	// TODO DWEBB is this necessary?
	double prev_cost = bestCost;
	double prev_counter = 0;

	while ((eps > 0.0 && (!middleFound || !rightFound)) /*&& (prev_counter < 5)*/) {
	//while ((eps > 0.0 && (!middleFound || !rightFound)) && (prev_counter < 5)) {
		// Compute cost at current eps
		integrateControlPolicy<_xDim,_uDim,_zDim>(experiment, step,
			L, l, eps, xBar, SigmaBar[0], uBar,
			xNext, SigmaNext, uNext, WNext);
		...
		//cost = computeExpectedCost(experiment, step, linearizeDynamics, quadratizeFinalCost, quadratizeCost, L, xNext, SigmaNext, uNext, WNext);
		cost = computeExpectedCost<_xDim,_uDim>(experiment, step, L, xNext, SigmaNext, uNext, WNext);

		// TODO DWEBB -- Doulbe check with Jur that this is okay
...
template <size_t _xDim, size_t _uDim, size_t _zDim>
inline void solvePOMDP(const Dynamics& experiment, double step,
	...
	int iterations = -1)
{
	...
	for (size_t t = 0; t < pathLen; ++t) {
		experiment.linearizeDynamics(step, xBar[t], uBar[t], xBar[t+1], A, B, M, COMPUTE_c|COMPUTE_A|COMPUTE_M);
		experiment.linearizeObservation(step, xBar[t+1], uBar[t], H, N);
	...
	while(!terminate)
	{
		backwardIteration<_xDim,_uDim,_zDim>(experiment, step, xBar, SigmaBar, uBar, L, l, gradient);
		//std::cout << "backwardIteration finished" << std::endl;
		
		//double prevCost = bestCost;
		forwardIteration<_xDim,_uDim,_zDim>(experiment, step, L, l, xBar, SigmaBar, uBar, WBar, bestCost, eps, gradient);
		//std::cout << "forwardIteration finished" << std::endl;
		...
		terminate = (eps*eps*absl == 0 || eps*eps*absl / absu < 1e-5);
		//terminate = (abs((prevCost - bestCost) / bestCost) < DBL_EPSILON);
