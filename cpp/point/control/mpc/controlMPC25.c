...
/*
 * Initializes a vector of length 48 with a value.
 */
void controlMPC_LA_INITIALIZEVECTOR_48(controlMPC_FLOAT* vec, controlMPC_FLOAT value)
{
	int i;
	for( i=0; i<48; i++ )
	{
...
/*
 * Initializes a vector of length 46 with a value.
 */
void controlMPC_LA_INITIALIZEVECTOR_46(controlMPC_FLOAT* vec, controlMPC_FLOAT value)
{
	int i;
	for( i=0; i<46; i++ )
	{
 ...
 */
void controlMPC_LA_INITIALIZEVECTOR_96(controlMPC_FLOAT* vec, controlMPC_FLOAT value)
{
	int i;
	for( i=0; i<96; i++ )
	{
 ...
 * This function is for vectors of length 96.
 */
void controlMPC_LA_DOTACC_96(controlMPC_FLOAT *x, controlMPC_FLOAT *y, controlMPC_FLOAT *z)
{
	int i;
	for( i=0; i<96; i++ ){
		*z += x[i]*y[i];
 ...
 * of length 48.
 */
void controlMPC_LA_VVADD3_48(controlMPC_FLOAT *u, controlMPC_FLOAT *v, controlMPC_FLOAT *w, controlMPC_FLOAT *z)
{
	int i;
	for( i=0; i<48; i++ ){
		z[i] = u[i] + v[i] + w[i];
...
/*
 * Vector subtraction z = -x - y for vectors of length 48.
 */
void controlMPC_LA_VSUB2_48(controlMPC_FLOAT *x, controlMPC_FLOAT *y, controlMPC_FLOAT *z)
{
	int i;
	for( i=0; i<48; i++){
		z[i] = -x[i] - y[i];
...
int controlMPC_LINESEARCH_BACKTRACKING_AFFINE(controlMPC_FLOAT *l, controlMPC_FLOAT *s, controlMPC_FLOAT *dl, controlMPC_FLOAT *ds, controlMPC_FLOAT *a, controlMPC_FLOAT *mu_aff)
{
    ...
    *a = mya;
    *mu_aff = mymu / (controlMPC_FLOAT)96;
    return lsIt;
}
...
*  and x,u,v are vectors of length 96.
 */
void controlMPC_LA_VSUB5_96(controlMPC_FLOAT *u, controlMPC_FLOAT *v, controlMPC_FLOAT a, controlMPC_FLOAT *x)
{
	int i;
	for( i=0; i<96; i++){
		x[i] = u[i]*v[i] - a;
 ...
 * Vector subtraction z = x - y for vectors of length 48.
 */
void controlMPC_LA_VSUB_48(controlMPC_FLOAT *x, controlMPC_FLOAT *y, controlMPC_FLOAT *z)
{
	int i;
	for( i=0; i<48; i++){
		z[i] = x[i] - y[i];
...
/*
 * Computes ds = -l.\(r + s.*dl) for vectors of length 96.
 */
void controlMPC_LA_VSUB7_96(controlMPC_FLOAT *l, controlMPC_FLOAT *r, controlMPC_FLOAT *s, controlMPC_FLOAT *dl, controlMPC_FLOAT *ds)
{
	int i;
	for( i=0; i<96; i++){
...
/*
 * Vector addition x = x + y for vectors of length 48.
 */
void controlMPC_LA_VADD_48(controlMPC_FLOAT *x, controlMPC_FLOAT *y)
{
	int i;
	for( i=0; i<48; i++){
		x[i] += y[i];
...
/*
 * Vector addition x = x + y for vectors of length 46.
 */
void controlMPC_LA_VADD_46(controlMPC_FLOAT *x, controlMPC_FLOAT *y)
{
	int i;
	for( i=0; i<46; i++){
		x[i] += y[i];
 ...
 */
void controlMPC_LA_VADD_96(controlMPC_FLOAT *x, controlMPC_FLOAT *y)
{
	int i;
	for( i=0; i<96; i++){
		x[i] += y[i];
...
controlMPC_FLOAT controlMPC_z[48];
controlMPC_FLOAT controlMPC_v[46];
controlMPC_FLOAT controlMPC_dz_aff[48];
controlMPC_FLOAT controlMPC_dv_aff[46];
controlMPC_FLOAT controlMPC_grad_cost[48];
controlMPC_FLOAT controlMPC_grad_eq[48];
controlMPC_FLOAT controlMPC_rd[48];
controlMPC_FLOAT controlMPC_l[96];
controlMPC_FLOAT controlMPC_s[96];
controlMPC_FLOAT controlMPC_lbys[96];
controlMPC_FLOAT controlMPC_dl_aff[96];
controlMPC_FLOAT controlMPC_ds_aff[96];
controlMPC_FLOAT controlMPC_dz_cc[48];
controlMPC_FLOAT controlMPC_dv_cc[46];
controlMPC_FLOAT controlMPC_dl_cc[96];
controlMPC_FLOAT controlMPC_ds_cc[96];
controlMPC_FLOAT controlMPC_ccrhs[96];
controlMPC_FLOAT controlMPC_grad_ineq[48];
...
info->it = 0;
controlMPC_LA_INITIALIZEVECTOR_48(controlMPC_z, 0);
controlMPC_LA_INITIALIZEVECTOR_46(controlMPC_v, 1);
controlMPC_LA_INITIALIZEVECTOR_96(controlMPC_l, 1);
controlMPC_LA_INITIALIZEVECTOR_96(controlMPC_s, 1);
info->mu = 0;
controlMPC_LA_DOTACC_96(controlMPC_l, controlMPC_s, &info->mu);
info->mu /= 96;
while( 1 ){
info->pobj = 0;
...
controlMPC_LA_DIAGZERO_MTVM_2_2(controlMPC_D01, controlMPC_v22, controlMPC_grad_eq23);
info->res_ineq = 0;
...
controlMPC_LA_VVADD3_48(controlMPC_grad_cost, controlMPC_grad_eq, controlMPC_grad_ineq, controlMPC_rd);
...
controlMPC_LA_DENSE_BACKWARDSUB_2(controlMPC_Ld22, controlMPC_yy22, controlMPC_dvaff22);
...
controlMPC_LA_DIAGZERO_MTVM_2_2(controlMPC_D01, controlMPC_dvaff22, controlMPC_grad_eq23);
controlMPC_LA_VSUB2_48(controlMPC_rd, controlMPC_grad_eq, controlMPC_rd);
...
controlMPC_LA_VSUB5_96(controlMPC_ds_aff, controlMPC_dl_aff, musigma, controlMPC_ccrhs);
...
controlMPC_LA_DENSE_MVMSUB1_2_2(controlMPC_Lsd22, controlMPC_yy21, controlMPC_beta22, controlMPC_bmy22);
controlMPC_LA_DENSE_FORWARDSUB_2(controlMPC_Ld22, controlMPC_bmy22, controlMPC_yy22);
controlMPC_LA_DENSE_BACKWARDSUB_2(controlMPC_Ld22, controlMPC_yy22, controlMPC_dvcc22);
...
controlMPC_LA_DIAGZERO_MTVM_2_2(controlMPC_D01, controlMPC_dvcc22, controlMPC_grad_eq23);
controlMPC_LA_VSUB_48(controlMPC_rd, controlMPC_grad_eq, controlMPC_rd);
...
controlMPC_LA_VSUB7_96(controlMPC_l, controlMPC_ccrhs, controlMPC_s, controlMPC_dl_cc, controlMPC_ds_cc);
controlMPC_LA_VADD_48(controlMPC_dz_cc, controlMPC_dz_aff);
controlMPC_LA_VADD_46(controlMPC_dv_cc, controlMPC_dv_aff);
controlMPC_LA_VADD_96(controlMPC_dl_cc, controlMPC_dl_aff);
controlMPC_LA_VADD_96(controlMPC_ds_cc, controlMPC_ds_aff);
